...
function submitform() {
	if (validate())
		$("#vlanApplyInfoAction3Phase").submit();
}
...
function validate() {
	...
	// 起始Vlan_Id范围
	if (parseInt($('#startId').val()) < 1 || parseInt($('#startId').val()) > 4094) {
		$.compMsg({
			type : 'error',
	...
	// 终止Vlan_Id范围
	if (parseInt($('#endId').val()) < 1 || parseInt($('#endId').val()) > 4094) {
		$.compMsg({
			type : 'error',
	...
	// 终止Vlan_Id与起始Vlan_Id比较
	if (parseInt($('#endId').val()) < parseInt($('#startId').val()) ) {
		$.compMsg({
			type : 'error',
			msg : '终止Vlan_ID的值必须大于等于起始Vlan_ID，请重新输入！'
		});
		$('#endId').select();
		return false;
	}
