$(function() {
	...
	// 只读
	$("select,:radio").attr("disabled", true);
	$("#mountDisk select,#mountDisk :radio").attr("disabled", false);
	// 自定义form样式
	$("select,:text").uniform();
	$("#hd_div :radio").uniform({
	...
	$.ajax({
		...
		success : function(data) {
			...
			case "error":
				...
				break;
			case "deleted":
				var errorMessage = diskNameOriginal + "已经被删除，跳回到详细信息列表页面";
				goBackForm(errorMessage, '');
				break;
			case "nopermission":
				var errorMessage = "用户没有操作" + diskNameOriginal + "的权限，跳回到列表页面";
				goBackForm(errorMessage, '');
				break;
			case "success":
				$("#diskSizeInput").val($("#upCustomName input[name='disk_size']").val());
				$("#diskSizeLabel").html($("#upCustomName input[name='disk_size']").val() + 'G');
				flag = true;
			case "failure":
				$("#customName").text($("[name='modify_custom_name']").val());
			...
			/*
			 * if (data.result == "error") { window.location.href =
			 * 'exceptionIntercepor.action'; } else { if (data.result ==
			 * "deleted") { var errorMessage = diskNameOriginal +
			 * "已经被删除，跳回到详细信息列表页面"; goBackForm(errorMessage, ''); } else { if
			 * (data.result == "nopermission") { var errorMessage = "用户没有操作" +
			 * diskNameOriginal + "的权限，跳回到列表页面"; goBackForm(errorMessage, ''); }
			 * else {
			 * $("#customName").text($("[name='modify_custom_name']").val());
			 * $("h2").html($("[name='modify_custom_name']").val()); } }
			 */
...
function upCustomName() {
	$("#upCustomName input[name='modify_custom_name']").val($("#customName").html());
...
function seachDiskList(data_val) {
	$("#vmList").html('<tr>' + '<th class="nl"></th>' + '<th>设备名称</th>' + '<th>ID</th>' + '</tr>');
	$.ajax({
		type : "POST",
		...
		success : function(data) {
			...
			for (var i = vmInfo.length - 1; i >= 0; i--) {
				if (vmInfo[i].resPoolId == resourcePoolId && vmInfo[i].resPoolPartId == resourcePoolPartId) {
					if (/*vmInfo[i].status != 'RUNNING' ||*/ vmInfo[i].vmId == null || vmInfo[i].vmId == '') {
						continue;
					}
					$("#vmList").append(
							'<tr id="tr_' + vmInfo[i].vmId + '">' + '<td align="center"><input type="radio" name="vh" value="' + vmInfo[i].vmId
									+ '"/></td>' + '<td>' + vmInfo[i].vmName + '</td>' + '<td>' + vmInfo[i].vmId + '</td>' + '</tr>');
				}
			}
			$("#vmList :radio").uniform({
				radioClass : 'radio'
			});
		},
		error : function() {
...
function mountDisk() {
	...
	$.dialog({
		...
		ok : function() {
			...
			var selectVmName = $("#tr_" + selectVmId.val()).children().eq(2).text();
						...
						$.ajax({
							...
							success : function(data) {
								if (data.result == "success") {
									$(".unMountDisk").show();
									$(".unMountDisk .detail-text").html(selectVmName);
									$(".mountDisk").hide();
									$(".status").removeClass('s-blue');
									$(".status").addClass('s-green');
									$(".status").html('已挂载');
									$.compMsg({
										type : 'success',
										msg : '挂载至主机' + selectVmName + '成功!'
									})
									tmpDialog.close();
...
function deleteDisk() {
	var diskId = $('#diskId').attr("value");
	var resourcePoolId = $('#resourcePoolId').attr("value");
	var resourcePoolPartId = $('#resourcePoolPartId').attr("value");
	var da_val = {
		diskId : diskId,
		resourcePoolId : resourcePoolId,
		resourcePoolPartId : resourcePoolPartId
	...
	$.ajax({
		url : '../console/diskDeleteAction.action',
		type : 'POST',
		data : da_val,
		cache : false,
		dataType : 'json',
...
function goBackForm(errMsg, msg) {
	$.backListMsg({
		msg : msg,
		errMsg : errMsg,
		url : 'ebsQueryListAction.action?businessId=' + businessId
	});
}

var goBackList = function() {

	var url = 'ebsQueryListAction.action?queryBusinessId=' + businessId;
	url += '&nodeType=' + $("#nodeType").val();
	url += '&nodeId=' + $("#nodeId").val();
	url += '&treeNodeName=' + $("#treeNodeName").val();
	url += '&pnodeId=' + $("#pnodeId").val();
	url += '&pnodeName=' + $("#pnodeName").val();
	url += '&appId=' + $("#appId").val();
	url += '&curFun=' + $("#curFun").val();
...
function queryStatusLoading() {
	var str = "";
	str = "[{id:'" + $("#diskId").val() + "',status:'" + $("#statusHidden").val() + "'}]";
	var da_val = {
		queryStr : str
	};
	$.ajax({
...
function showStatus(status) {
	if (status == "0") {
		$("#status").text("待创建").removeClass().addClass("status mt s-blue");
	}
	if (status == "1") {
		$("#status").text("创建中").removeClass().addClass("status mt s-gray");
	}
	if (status == "2") {
		$("#status").text("已创建").removeClass().addClass("status mt s-blueGreen");
	}
	if (status == "4") {
		$("#status").text("已挂载");
		$("#status").removeClass().addClass("status mt s-green");
	}
	if (status == "5") {
		$("#status").text("未挂载").removeClass().addClass("status mt s-blue");
	}
	if (status == "6") {
		$("#status").text("创建失败");
		$("#status").removeClass().addClass("status mt s-orange");
	}
	if (status == "7") {
		$("#status").text("挂载失败");
		$("#status").removeClass().addClass("status mt s-orange");
