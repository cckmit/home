/*******************************************************************************
 * @(#)VmBakListQueryImpl.java 2015-3-10
 *
 ...
 */
public class VmBakListQueryImpl implements VmBakListQuery,Serializable{
	...
	private static LogService logger = LogService.getLogger(VmBakListQueryImpl.class);
     ...
     */
    @Override
    public RPPVMBakListQueryResp queryVmBakList(RPPVMBakListQueryReq vmBakReq) {
    ...
    private RuntimeContext assembleRuntimeContext(RPPVMBakListQueryReq vmBakReq) {
    	RuntimeContext req = new RuntimeContext();
        req.setAttribute(RPPBaseReq.REQ_BODY, vmBakReq);
        return req;
    }

    private RPPVMBakListQueryResp assembleResp(HttpSyncRespData resp) {
    	RPPVMBakListQueryResp vmBakResp = (RPPVMBakListQueryResp) resp.getRuntimeContext().getAttribute(RPPBaseResp.RESP_BODY);
        return vmBakResp;
    }

    private RPPVMBakListQueryResp assmbleErrorResp(CommonStatusCode code, String errorMessage) {
    	RPPVMBakListQueryResp vmBakResp = new RPPVMBakListQueryResp();
