/*******************************************************************************
 * @(#)VmBakStatusHandler.java 2013-1-17
 *
...
/**
 * 枚举类型VmBakStatus映射处理类，存储时把VmBakStatus映射成数据库需要的字段值，读取时把数据库中的字段值映射成VmBakStatus
 * @author <a href="mailto:yuan.x@neusoft.com">yuanxue</a>
 * @version $Revision 1.0 $ 2014-1-14 下午03:02:47
 */
public class VmBakStatusHandler implements TypeHandlerCallback {

    @Override
    public Object getResult(ResultGetter getter) throws SQLException {
        if (null == getter.getObject()) {
            throw new IllegalArgumentException("Count not convert null " + " to enum VmBakStatusCode!");
        }
    ...
    private Object getStatus(String value) {
        Object result = null;
        for (VmBakStatus status : VmBakStatus.values()) {
    ...
    public void setParameter(ParameterSetter setter, Object parameter) throws SQLException {
        if (null == parameter) {
            ...
            setter.setString(((VmBakStatus) parameter).getCode());
        }
