...
public class SnapshotCreateImpl implements SnapshotCreate {

	private static LogService logger = LogService
			.getLogger(SnapshotCreateImpl.class);
	...
	@Override
	public RPPApplySnapshotResp create(RPPApplySnapshotReq snapshotReq) {
		RuntimeContext req = new RuntimeContext();
		req.setAttribute(RPPBaseReq.REQ_BODY, snapshotReq);

		HttpSyncRespData resp = null;
		try {
			resp = senderEntry.sendHttpRequest(req, url, httpTimeOut,
					receiveTimeout);
		} catch (IOException e) {
		...
		return (RPPApplySnapshotResp) resp.getRuntimeContext().getAttribute(
				RPPBaseResp.RESP_BODY);

	}

	private RPPApplySnapshotResp assmbleErrorResp(CommonStatusCode code,
			String errorMessage) {
		RPPApplySnapshotResp snapshotResp = new RPPApplySnapshotResp();
		snapshotResp.setResultCode(code.toString());
		snapshotResp.setResultMessage(errorMessage);
		return snapshotResp;
	}
