 ...
 */
public class VMOnloadItemAction extends BaseAction {

    private static final long serialVersionUID = -6356197450412064816L;

    private static LogService logger = LogService.getLogger(VMOnloadItemAction.class);
    ...
    private List<ItemInfo> queryItems(String respoolId, String respoolPartId) throws SQLException {
        ...
        standards = (List<StandardInfo>) ibatisDAO.getData("queryStandards", rp);

        for (int i = 0; i < standards.size(); i++) {
            StandardInfo standardInfo = standards.get(i);
            List<ItemInfo> itemsList = (List<ItemInfo>) ibatisDAO.getData("queryItems",
                    standardInfo);
