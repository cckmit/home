...
public class VmBakDelImpl implements VmBakDel ,Serializable{
    ...
    @Override
    public RPPVMBakDeleteResp delVmBak(RPPVMBakDeleteReq vmBakReq) {
        RuntimeContext req = this.assembleRuntimeContext(vmBakReq);
        HttpSyncRespData resp = null;
        try {
        	if (logger.isDebugEnable()) {
                logger.debug("删除虚拟机备份信息接口准备发送");
    ...
    private RuntimeContext assembleRuntimeContext(RPPVMBakDeleteReq vmBakReq) {
    	RuntimeContext req = new RuntimeContext();
        req.setAttribute(RPPBaseReq.REQ_BODY, vmBakReq);
        return req;
    }

    private RPPVMBakDeleteResp assembleResp(HttpSyncRespData resp) {
    	RPPVMBakDeleteResp vmBakResp = (RPPVMBakDeleteResp) resp.getRuntimeContext().getAttribute(RPPBaseResp.RESP_BODY);
        return vmBakResp;
    }

    private RPPVMBakDeleteResp assmbleErrorResp(CommonStatusCode code, String errorMessage) {
    	RPPVMBakDeleteResp vmBakResp = new RPPVMBakDeleteResp();
