/*******************************************************************************
 * @(#)VmBakStatusQueryImpl.java 2015-3-10
 *
 ...
 */
public class VmBakStatusQueryImpl implements VmBakStatusQuery,Serializable{
	...
	private static LogService logger = LogService.getLogger(VmBakStatusQueryImpl.class);
     ...
     */
    @Override
    public RPPVMBakTaskQueryResp queryVmBakStatus(RPPVMBakTaskQueryReq vmBakReq) {
        RuntimeContext req = this.assembleRuntimeContext(vmBakReq);
        HttpSyncRespData resp = null;
        try {
            if (logger.isDebugEnable()) {
                logger.debug("查询虚拟机备份信息接口准备发送");
        ...
        if (logger.isDebugEnable()) {
            logger.debug("查询虚拟机备份信息接口发送成功");
        }
        return this.assembleResp(resp);
    }

    private RuntimeContext assembleRuntimeContext(RPPVMBakTaskQueryReq vmBakReq) {
    	RuntimeContext req = new RuntimeContext();
        req.setAttribute(RPPBaseReq.REQ_BODY, vmBakReq);
        return req;
    }

    private RPPVMBakTaskQueryResp assembleResp(HttpSyncRespData resp) {
    	RPPVMBakTaskQueryResp vmBakResp = (RPPVMBakTaskQueryResp) resp.getRuntimeContext().getAttribute(RPPBaseResp.RESP_BODY);
        return vmBakResp;
    }

    private RPPVMBakTaskQueryResp assmbleErrorResp(CommonStatusCode code, String errorMessage) {
    	RPPVMBakTaskQueryResp vmBakResp = new RPPVMBakTaskQueryResp();
