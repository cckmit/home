 ...
 * @version $Revision 1.0 $ 2015年3月5日 上午10:20:12
 */
public class VLANApplyInfoAction extends ResourceManagementBaseAction {

    private static final long serialVersionUID = -3246558237673423698L;

    private static LogService logger = LogService.getLogger(VLANApplyInfoAction.class);
    ...
    public String execute() {
        ...
        if (resp.getResultCode().equals(SUCCEESS_CODE)) {
            ...
            try {
                ...
                // 更新数据库
                updateBatchVO.add(new BatchVO("ADD", "createVLANInstanceInfo", vlanInfo));
                if (logger.isDebugEnable()) {
                ...
                logger.error(VlanStatusCode.createVlanOrderException,
                        getText("vlan.applyinfo.fail"), e2);
                this.addActionError(getText("vlan.applyinfo.fail"));
                errMsg = getText("vlan.applyinfo.fail");
            ...
            this.addActionError(getText("vlan.applyinfo.sendcreateEBS.fail"));
            errMsg = getText("vlan.applyinfo.sendcreateEBS.fail");
            return ConstantEnum.FAILURE.toString();
        }
    ...
    private void assembleReq(RPPVlanApplyReq vlanReq) {
        ...
        //vlanReq.setResourcePoolPartId(respoolPartId);
        vlanReq.setCount(1);
