...
<div class="list">
	...
	<div class="details-con">
		<div
			class="detail-title <s:if test="LBinfo.status==0">blue</s:if>
		        			<s:if test="LBinfo.status==1">green</s:if>
		        			">
			<div class="status ">
				<span id="status"> <s:if test="LBinfo.status == 0">待创建</s:if>
					<s:elseif test="LBinfo.status == 1">运行中</s:elseif></span> <input
					type="hidden" id="LBinfostatus"
					value="<s:property value="LBinfo.status" />" />
			</div>
			<h2>
...
<!-- 			<tr>
				<td>所属业务：</td>
				<td id="bindDiv"><a href="javascript:bindBusiness();">绑定业务</a></td>
			</tr>    -->
