					...
					function(e) {
						if (e.target !== slider) {
							if (e.offsetX > (wrapper_width - slider_width)) {
								...
								$('#discSize')
										.val(
												((e.offsetX / wrapper_width) * 5).toFixed(1)
														+ "TB");
								$('#disk')
										.val(((e.offsetX / wrapper_width) * 5).toFixed(1));
								$('.cart-bar span#cart-discSize')
										.text(((e.offsetX / wrapper_width) * 5).toFixed(1)
														+ "TB");
							}
							// 计费
							charging();
					...
					function(e) {
						if (drag == 1) {
							if ((e.pageX - wrapper_left - menu_width) > (wrapper_width - slider_width / 2)) {
								...
								$('#discSize').val("5TB");
								$('#disk').val("5");
								$('.cart-bar span#cart-discSize').text("5TB");
							} else {
								slider.style.left = (e.pageX - wrapper_left - menu_width)
										- slider_width + 'px';
								fill.style.width = (e.pageX - wrapper_left - menu_width)
								...
								$('#discSize')
										.val((((e.pageX
																- wrapper_left - menu_width) / wrapper_width) * 5 ).toFixed(1)
														+ "TB");
								$('#disk')
								.val((((e.pageX
												- wrapper_left - menu_width) / wrapper_width) * 5).toFixed(1)
...
function charging() {
	var a = 10000000;
	var diskSize = parseInt($("#discSize").val().replace("GB", ""));
