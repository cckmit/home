$(function() {
	...
	$('div.cart-bar').load('../cart-bar/ebs_cart.jsp',function(response, status, xhr) {
		$('span.total-price >span:first-child').removeClass("none");
		$('a.apply').css('top',0);
...
function charging() {
	var diskSize = $("#discSize").val().replace("TB", "");
	...
	var price = '0.00';
	if (chargeType == "h") {
		price = hourPrice * diskSize * num * time * 1024;
	} else if (chargeType == "m") {
		price = monthPrice * a * diskSize * num * time * 1024;
	}
	if (price == '0') {
		$(".price").text("￥" + 0.00);
	} else {
