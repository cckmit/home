		...
		$.ajax({
			...
			success : function(data) {
				...
				$("#bindDiv1").html(
						...
						+ '<input type="hidden" id="appId1" name="fsInstanceInfo.appId" value="' + data.businessInfoList[0].businessId + '"/>');
			}
		});
	}
...
function bindBusiness() {
	...
	$.dialog({
		...
		ok : function() {
			...
			if (!$("#appId").val()) {
				$('#bindDiv').append(
						'<input type="hidden" id="appId" name="fsInstanceInfo.appId" value="'
								+ appId + '"/>');
			} else {
				$("#appId").val(appId);
			}
