            ...
            <div class="login-float-top">
            		...
            		<form id="formSubmit" name="formSubmit" method="post" action="forgetPassword.action" >
						<div id="detail_tab_info" class="login-detail-font">
							<div><span><font class="red-font">*</font>用户帐号:</span><s:textfield id="userId" name="userId"/></div>
							<div><span><font class="red-font">*</font>找回方式：</span>
								<input type="radio" id="shortMessage" name="radio" checked="checked" value="shortMessage"/><label>短信</label>
								<input type="radio" id="email" name="radio" value="email"/><label>邮件</label>	
							</div>
							<div><font class="red-font"><s:property value="errMsg" /><label id="errParam"></label></font></div>
							<div class="login-detail-btn"><input type="submit" class="submit" onclick="submitform();return false;" value="找回密码" /><!--<span ><a href="javascript:submitform()">登录</a></span>--> <!--<input type="submit" value="登录" />--> </div>
						</div>
					</form>
        	</div>
        </div>
