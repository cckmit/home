...
<script type="text/javascript" src="../scripts/pagesjs/report/hstDeviceReport.js"></script>
<script type="text/javascript">
	...
	$(function() {
		...
		var yAxisTitle = "";
		var unit = "";
		if(deviceType =='2'){ // 防火墙统计的是读吞吐量
			chartTitle = '<b>TOP10-' + deviceTypeName + '吞吐量平均值<b>';
			yAxisTitle = deviceTypeName + '吞吐量平均值(MB/S)';
			unit = 'MB/S';
		} else if(deviceType =='41'){ // 交换机端口显示包总数
			chartTitle = '<b>TOP10-' + deviceTypeName + '端口字节总数平均值<b>';
			yAxisTitle = deviceTypeName + '端口字节总数平均值';
		}
...
// 					}
		...
		var deviceType = '<s:property value="devicePerformanceInfo.deviceType" />';
        if(deviceType == '3'){ // 阵列
        	performanceType = 'diskReadBytes';
        } else if(deviceType =='41' || deviceType =='51'){ // 交换机和路由器端口显示包总数
        	performanceType = 'pkts';
		} else{
        	performanceType = 'cpu';
        }
		...
		<div class="BussTitle">
			...
			<div class="BussTitleButton" style="width:350px">
				<s:if test="devicePerformanceInfo.deviceType == 3"><!-- 阵列 -->
					<a id="cpuUrl" href="#" class="BussTitleButtonFocus" onclick="setPerformanceUrl(<s:property value="devicePerformanceInfo.deviceType" />, '1', 'cpu')"><div id="cpuDiv" class="">读吞吐量</div></a>
					<a id="memUrl" href="#" class="BussTitleButtonLoseFocus" onclick="setPerformanceUrl(<s:property value="devicePerformanceInfo.deviceType" />, '1', 'mem')"><div id="memDiv" class="">写吞吐量</div></a>
				</s:if>
				<s:elseif test="devicePerformanceInfo.deviceType == 2"> <!-- 防火墙 -->
					...
					<a id="memUrl" href="#" class="BussTitleButtonLoseFocus" onclick="setPerformanceUrl(<s:property value="devicePerformanceInfo.deviceType" />, '1', 'mem')"><div id="memDiv" class="">内存</div></a>
					<a id="hstUrl" href="#" class="BussTitleButtonFocus" onclick="setPerformanceUrl(<s:property value="devicePerformanceInfo.deviceType" />, '1', 'hst')"><div id="hstDiv" class="">吞吐量</div></a>
				</s:elseif>
				<s:elseif test="devicePerformanceInfo.deviceType == 41"><!-- 交换机端口 -->
					...
					<a id="hstUrl" href="#" class="BussTitleButtonFocus" onclick="setPerformanceUrl(<s:property value="devicePerformanceInfo.deviceType" />, '1', 'hst')"><div id="hstDiv" class="">字节总数</div></a>
				</s:elseif>
				...
				<s:else>
					<a id="cpuUrl" href="#" class="BussTitleButtonFocus" onclick="setPerformanceUrl(<s:property value="devicePerformanceInfo.deviceType" />, '1', 'cpu')"><div id="cpuDiv" class="">CPU</div></a>
					<a id="memUrl" href="#" class="BussTitleButtonLoseFocus" onclick="setPerformanceUrl(<s:property value="devicePerformanceInfo.deviceType" />, '1', 'mem')"><div id="memDiv" class="">内存</div></a>
				</s:else>
			</div>
		</div>
