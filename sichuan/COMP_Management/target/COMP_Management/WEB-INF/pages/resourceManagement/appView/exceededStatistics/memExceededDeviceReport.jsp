...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#chartPer")[0] = new Charts.Chart({
			...
			tooltip : {
				formatter : function() {
					...
					s += '占比: ' + this.point.per + '%<br/>';
					if (this.series.name != "其它") {
						s += '平均使用率: ' + this.point.ave + '%<br/>';
						s += '平均可用内存: ' + this.point.freeAve + 'GB';
					}
					return s;
				}
			},
		...
		$("#chartInt")[0] = new Charts.Chart({
			...
			tooltip : {
				formatter : function() {
					...
					if (this.series.name != "其它") {
						s += '平均使用率: ' + this.point.ave + '%<br/>';
						s += '平均可用内存: ' + this.point.freeAve + 'GB<br/>';
					}
		...
		/****************************chartInt DEMO end************************************************/
		getExceededData('/memExceededPerListByAjax.action?deviceOverInfo.deviceType=' + '<s:property value="deviceOverInfo.deviceType" />',
				'ExceededTbody', 'pageBarPerDiv', PerTbodyAccess);

		getExceededData('/memExceededNumListByAjax.action?deviceOverInfo.deviceType=' + '<s:property value="deviceOverInfo.deviceType" />',
	...
	<div class="details-con">
		...
		<div class="BussHardsoft">
			...
			<div class="BussIllustrate">
				<b>未超标</b>：内存使用率&lt=70% <b>超标</b>：内存使用率&gt70%
			</div>
		</div>
