...
function setTbody(data, deviceType) {
	...
	$.each(data.monthPerformanceInfolist,
			function(index) {
				// 物理机
				if (deviceType == 0) {
					$('<tr>').appendTo('#listTbody').append(
							...
							'<td>' + this.cpuNum + 'C, ' + this.memorySize
									+ 'GB</td>').append(
							'<td title = "' + this.os +'">' + getTooLongStr(this.os) + '</td>').append(
							'<td>' + this.appName + '</td>').append(
							'<td>' + this.appContacts + '</td>')
							.append(
									'<td>' + this.cpuProcessorUtilization
											+ '%</td>').append(
									'<td>' + this.maxCpuProcessorUtilization
											+ '%</td> ');
				} else {
					$('<tr>').appendTo('#listTbody').append(
							'<td>' + this.deviceName + '</td> ').append(
