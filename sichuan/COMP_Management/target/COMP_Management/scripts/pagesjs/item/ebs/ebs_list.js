$(function(){
    ...
    $("#apply_host").click(function (){
        window.location.href = 'ebsItemCreateInfo.action';
    });
...
function onItemModify(obj, itemId){
    if (itemId != '') {
        $("#itemId").val(itemId);
        $("#itemModify").attr('action', 'ebsItemInfo.action');
        $("#itemModify").submit();
    }
}
...
function onItemDetail(itemId){
    if (itemId != '') {
        $("#itemId").val(itemId);
        $("#itemModify").attr('action', 'ebsItemDetail.action');
        $("#itemModify").submit();
    }
}
			...
			if(data!=null){
				...
				for(var i=0;i< list.length;i++){
					...
					if(statusCode!='5' && statusCode!='7'){
	                	...
	                	willHtml+='<a href="javascript:void(0)" onclick="onItemDetail(\''+entity.itemId+'\');">详情</a>';
						if (statusCode == '0' || statusCode == '3') {
							willHtml+='<a href="javascript:void(0);"  onclick="onItemModify(this,\''+entity.itemId+'\');">修改</a>';
						}
						if (statusCode == '0') {
	                    	willHtml+='<a href="javascript:void(0)" onclick="onItemSubmit(this,\''+entity.itemId+'\');">提交</a>';
						}
						if (statusCode == '2') {
	                    	willHtml+='<a href="javascript:void(0);" onclick="onItemSend(this,\''+entity.itemId+'\')">发布</a>';
						}
						if (statusCode == '0') {
							willHtml+='<a href="javascript:void(0);" onclick="onItemDel(this,\''+entity.itemId+'\')">删除</a>';
						}
		                willHtml+='</td>';
		              	willHtml+='</tr>';
	                	...
	                	doneHtml+='	<a href="javascript:void(0)" onclick="onItemDetail(\''+entity.itemId+'\');">详情</a>';
						if(statusCode!='7'){
	                    	doneHtml+='<a href="javascript:void(0);" onclick="onItemDown(this,\''+entity.itemId+'\')">下架</a>';
						}
						if (recommendCode== '0'&& statusCode!='7') {
							doneHtml+='<a href="javascript:void(0);" onclick="onItemCommend(this,\''+entity.itemId+'\')">推荐</a>';
						}
						if (recommendCode=='1'&& statusCode!='7') {
	                    	doneHtml+='<a href="javascript:void(0);" onclick="onItemCommend(this,\''+entity.itemId+'\')">取消推荐</a>';
						}
		                doneHtml+='</td>';
		              	doneHtml+='</tr>';
					}
