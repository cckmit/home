...
function getJsonStr(opt){
	...
	var da_val = {'itemJson':jsonStr,'itemId':itemId,'itemName':itemName,'catalogId':catalogId,
				  'maxNum':maxNum,'minNum':minNum,'sellStartTime':sellStartTime,
				  'sellEndTime':sellEndTime,'description':description,'oldStandardId':oldStandardId,
...
function saveItem(opt){
	$("#saveItem").hide();
	$("#saveAndSubmitItem").hide();
	if(saveValidation(opt)){
		$("#saveItem").show();
		$("#saveAndSubmitItem").show();
		return false;
	}
	var urls = "";
	if(opt == 'update'){
		urls = 'vmbakItemUpdate.action?struts.enableJSONValidation=true';
	}else if(opt == 'save'){
		urls = 'vmbakItemsCreate.action?struts.enableJSONValidation=true';
	}
...
function saveAndSubmitItem(opt){
	...
	var urls = "";
	if(opt == 'update'){
		urls = 'vmbakItemUpdateAndSubmit.action?struts.enableJSONValidation=true';
	}else if(opt == 'save'){
		urls = 'vmbakItemCreateAndSubmit.action?struts.enableJSONValidation=true';
	}
	$.ajax({
        type: "POST",
        url: urls,
