$(function(){
    ...
    $("#apply_host").click(function (){
        window.location.href = 'vmbakItemCreateInfo.action';
    });
...
function onItemModify(obj, itemId){
    if (itemId != '') {
        $("#itemId").val(itemId);
        $("#itemModify").attr('action', 'vmbakItemInfo.action');
        $("#itemModify").submit();
    }
}
...
function onItemDetail(itemId){
    if (itemId != '') {
        $("#itemId").val(itemId);
        $("#itemModify").attr('action', 'vmbakItemDetail.action');
        $("#itemModify").submit();
    }
}
var willTables = '<col style="width:5%;" />';
    ...
    willTables += '<col style="width:15%;" />';
    willTables += '<col style="width:16%;" />';
    willTables += '<col style="width:32%;" />';
  	willTables += '<tr>';
  	willTables += '<th class="nl" style="width:5%;"><input type="checkbox"></th>';
    willTables += '<th style="width:18%;">条目名称</th>';
    willTables += '<th style="width:15%;">条目状态</th>';
    willTables += '<th style="width:15%;">创建人</th>';
    willTables += '<th style="width:16%;">创建时间</th>';
    willTables += '<th style="width:32%;">操作</th>';
  	willTables += '</tr>';
var doneTables = '<col style="width:5%;" />';
    doneTables += '<col style="width:18%;" />';
    doneTables += '<col style="width:15%;" />';
    doneTables += '<col style="width:15%;" />';
    doneTables += '<col style="width:16%;" />';
    doneTables += '<col style="width:32%;" />';
	doneTables += '<tr>';
    doneTables += '<th class="nl" style="width:5%;"><input type="checkbox"></th>';
    doneTables += '<th style="width:18%;">条目名称</th>';
    doneTables += '<th style="width:15%;">条目状态</th>';
    doneTables += '<th style="width:15%;">创建人</th>';
    doneTables += '<th style="width:16%;">创建时间</th>';
    doneTables += '<th style="width:32%;">操作</th>';
	doneTables += '</tr>';
			...
			if(data!=null){
				...
				for(var i=0;i< list.length;i++){
					...
					if(statusCode!='5' && statusCode!='7'){
						...
						if (statusCode != '1' && statusCode != '4' &&statusCode != '7' &&statusCode != '8') {
