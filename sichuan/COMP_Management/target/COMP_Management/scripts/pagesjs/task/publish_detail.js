...
//菜单显示当前，开发时删除
	...
	$("#publishPass").click(function (){
		publishPass($("#itemId").val());
	});
	$("#publishUnPass").click(function (){
		publishNuPass($("#itemId").val());
	});
	fomartDate();//格式化日期
});
//格式化日期
...
function backList(msg,errMsg){
	$.backListMsg({msg:msg,errMsg:errMsg,url:'publishItems.action'});
}
//加载虚拟机规格
function vmStandard(){
