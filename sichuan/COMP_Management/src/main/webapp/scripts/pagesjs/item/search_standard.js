...
tables += '<th style="width:20%;">CPU数量(个)</th>';
tables += '<th style="width:20%;">内存容量(MB)</th>';
tables += '<th style="width:30%;">磁盘空间(GB)</th>';
tables += '</tr>';
//资源规格查询
function searchStandard(){
	var queryCpuNum = $.trim($("#queryCpuNum").val());
	var queryDiscSize = $.trim($("#queryDiscSize").val());
	var queryRamSize = $.trim($("#queryRamSize").val());
	var queryRecommend = $.trim($("#queryRecommend").val());
	var da_val = {"cpuNum":queryCpuNum,"discSize":queryDiscSize,"ramSize":queryRamSize,"useStatus":queryRecommend};
    $.ajax({
        type: "POST",
        ...
        success: function(data){
			...
			if(!jQuery.isEmptyObject(data)){
				...
				$("#standardlist").empty();
				var str = tables;
				str+='<tr><td colspan="5" align="center">暂无符合数据！</td></tr>';
				$("#standardlist").append(str);
			}
        }
    });
...
function standardFieldErrors(errors){
	...
	if (!jQuery.isEmptyObject(errors)) {
		if (!jQuery.isEmptyObject(errors.cpuNum)) {
			mes += errors.cpuNum + '\n';
		}
