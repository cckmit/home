...
function setTbody(data,timeType,deviceType) {
	...
	$.each(data.devicePerformanceInfolist, function(index) {
		if(deviceType == '3'){
			...
			.append('<td>' + this.hstDiskReadBytes + '</td> ')
			.append('<td>' + this.hstDiskWriteBytes + '</td>')
			...
			chart : {
				...
				events: {
					...
					load: function() {
						...
						if(deviceType == '3'){
							//初始化
							var actionStr = '/devicePerformanceRTDetailByAjax.action';//读吞吐量、写吞吐量列表数据
...
function searchTimeTypeReport(deviceType, timeType){
	var form = $("#searchForm");
	$("#searchForm").attr("action", "cpuDeviceTop10PerformanceAction.action?devicePerformanceInfo.deviceType=" + deviceType + "&devicePerformanceInfo.timeType=" + timeType);
	form.submit();
}
