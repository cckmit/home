 ...
 */
public class MemHistoryPerformanceOverAction extends HistoryBaseAction {

	private static final long serialVersionUID = 1L;

	private static Logger logger = Logger.getLogger(CpuHistoryPerformanceAction.class);

	/** 内存使用率超标设备统计图表的横纵坐标数值 */
	...
	public String execute() {
		logger.info("业务性能统计-内存使用率超标设备统计，开始");
		...
		try {
			if (null != historyPerformanceInfo.getStaFlag() && !"".equals(historyPerformanceInfo.getStaFlag())) {
				chartDatalist = ibatisDAO.getData("getMemHistoryPerformanceOver", historyPerformanceInfo);
			}

			appList = ibatisDAO.getData("getAppList", null);
		} catch (SQLException e) {
			logger.error("业务性能统计-内存使用率超标设备统计，操作数据库异常", e);
			e.printStackTrace();
		}
		...
		logger.info("业务性能统计-内存使用率超标设备统计，结束");
	 ...
	 */
	@SuppressWarnings("unchecked")
	public String getMemHistoryPerformanceOverByAjax() {
		/* 查询日期处理 */
		this.formatDateForSql();
		
		if (null != historyPerformanceInfo.getStaFlag() && !"".equals(historyPerformanceInfo.getStaFlag())) {
			historyPerformanceInfolist = getPage("getHistoryPerformanceCount",
					"getMemHistoryPerformanceOver", historyPerformanceInfo,
