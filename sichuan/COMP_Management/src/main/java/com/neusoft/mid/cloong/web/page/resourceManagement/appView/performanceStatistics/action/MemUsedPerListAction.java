 ...
 */
public class MemUsedPerListAction extends ResourceManagementBaseAction
{

	private static final long serialVersionUID = -5024421508278442852L;

	private static Logger logger = Logger.getLogger(MemUsedPerListAction.class);
	...
	public String execute()
	{
		...
		//查询内存使用率列表，async异步，sync同步
		if(device_type.equals("2")){
			performancelist = getPage("queryPmMemUsedPerListCount" , "queryPmMemUsedPerList",
					staDeviceWeek,PageTransModel.ASYNC,PageToDisplayPerModel.PAGESIZE_10_DISPLAY);
		}else if(device_type.equals("3")){
			performancelist = getPage("queryVmMemUsedPerListCount" , "queryVmMemUsedPerList",
					staDeviceWeek,PageTransModel.ASYNC,PageToDisplayPerModel.PAGESIZE_10_DISPLAY);
		}else if(device_type.equals("0")){
			performancelist = getPage("queryMiniPmMemUsedPerListCount" , "queryMiniPmMemUsedPerList",
					staDeviceWeek,PageTransModel.ASYNC,PageToDisplayPerModel.PAGESIZE_10_DISPLAY);
		}else if(device_type.equals("1")){
			performancelist = getPage("queryMiniPmParMemUsedPerListCount" , "queryMiniPmParMemUsedPerList",
					staDeviceWeek,PageTransModel.ASYNC,PageToDisplayPerModel.PAGESIZE_10_DISPLAY);
		}
