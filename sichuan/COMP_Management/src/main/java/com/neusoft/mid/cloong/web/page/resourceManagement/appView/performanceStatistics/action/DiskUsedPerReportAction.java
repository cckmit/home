 ...
 */
public class DiskUsedPerReportAction extends ResourceManagementBaseAction
{

	private static final long serialVersionUID = -5024421508278442852L;

	private static Logger logger = Logger.getLogger(DiskUsedPerReportAction.class);
    ...
    private String device_typeNameForList;//页面图显示设备类型名称用
	...
	{
		//查询磁盘使用率Top10图表数据
		List<StaDeviceWeek> diskTop10List = null;
		...
		try
		{
			...
			if(device_type.equals("2")){
				diskTop10List = ibatisDAO.getData("getPmDiskUsedPerListTop10" , staDeviceWeek);
				device_typeNameArr.add("物理机".toString());
				device_typeNameForList = "物理机";
			}else if(device_type.equals("3")){
				diskTop10List = ibatisDAO.getData("getVmDiskUsedPerListTop10" , staDeviceWeek);
				device_typeNameArr.add("虚拟机".toString());
				device_typeNameForList = "虚拟机";
			}else if(device_type.equals("0")){
				diskTop10List = ibatisDAO.getData("getMiniPmDiskUsedPerListTop10" , staDeviceWeek);
				device_typeNameArr.add("小型机".toString());
				device_typeNameForList = "小型机";
			}else if(device_type.equals("1")){
				diskTop10List = ibatisDAO.getData("getMiniPmParDiskUsedPerListTop10" , staDeviceWeek);
		...
		{
			logger.error("查看虚拟机性能统计之内存使用率，操作数据库异常" , e);
			e.printStackTrace();
		}
		...
		for (StaDeviceWeek i : diskTop10List)
		{
			x_axisArr.add(i.getDevice_name());
			
			y_obj.put("y" , Float.parseFloat(i.getDisk_used_per()));
