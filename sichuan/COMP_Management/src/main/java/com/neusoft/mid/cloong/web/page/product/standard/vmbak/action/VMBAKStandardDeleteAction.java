/*******************************************************************************
 * @(#)VMBAKStandardDeleteAction.java 2014年1月21日
 *
 ...
 * @version $Revision 1.0 $ 2014年1月21日 下午3:45:28
 */
public class VMBAKStandardDeleteAction extends BaseStandardDeleteAction {

    /**
     * serialVersionUID
     */
    private static final long serialVersionUID = -4691035829029126367L;

    private static LogService logger = LogService.getLogger(VMBAKStandardDeleteAction.class);
    ...
    public String execute() {
        ...
        if (standardNumInUse > 0) {
            ...
            logger.info("ID为" + standardId + "的备份规格正在使用，不允许删除");
            return ConstantEnum.SUCCESS.toString();
        }
    ...
    /**
     * updateData 更新数据(删除虚拟机备份规格信息\删除同步表信息)
    ...
    private void updateDataBatch(String standardId,
        ...
        batchVOs.add(new BatchVO("MOD", "deleteVMBAKStandard", standardId));
        ...
        try {
            ...
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION,
                    "删除虚拟机备份规格失败！原因：调用同步删除接口后，查询规格是否存在未同步过时，数据库异常。", e);
            resultPath = ConstantEnum.ERROR.toString();
        } catch (Exception e) {
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION,
                    "删除虚拟机备份规格失败！原因：调用同步删除接口后，查询规格是否存在未同步过时，数据库异常。", e);
    ...
    /***
     * updateDataStandard 未同步过,删除虚拟机备份规格
     * @param standardId 规格ID
     */
    public void updateDataStandard(String standardId) {
        ...
        try {
            ...
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION,
                    "删除虚拟机备份规格失败！原因：查询规格是否存在未同步过时，数据库异常。", e);
            resultPath = ConstantEnum.ERROR.toString();
        } catch (Exception e) {
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION,
                    "删除虚拟机备份规格失败！原因：查询规格是否存在未同步过时，数据库异常。", e);
