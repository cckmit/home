 ...
 */
public class VMStandardDeleteAction extends BaseStandardDeleteAction {

    private static final long serialVersionUID = -1441697968480674248L;

    private static LogService logger = LogService.getLogger(VMStandardDeleteAction.class);
    ...
    public String execute() {
        ...
        try {
            ...
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION,
                    "删除虚拟机规格失败！原因：查询规格是否被条目关联使用时，数据库异常。", e);
    ...
    /**
     * updateData 更新数据(删除规格信息\删除同步表信息\删除规格与镜像关联关系表)
     * 【调用接口后的更新】
    ...
    private void updateDataBatch(String standardId,
        ...
        try {
            ...
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION,
                    "删除虚拟机规格失败！原因：调用同步删除接口后，查询规格是否存在未同步过时，数据库异常。", e);
            resultPath = ConstantEnum.ERROR.toString();
        } catch (Exception e) {
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION,
                    "删除虚拟机规格失败！原因：调用同步删除接口后，查询规格是否存在未同步过时，数据库异常。", e);
        ...
        batchVOs.add(new BatchVO("MOD", "deleteVmStandard", standardId));
        if (standardSynchronizeDeleteResps.getFailure().size() > 0) {
            Map<String, Object> updateMaps = new HashMap<String, Object>();
            updateMaps
                    .put("StandardSynchronizeResult", standardSynchronizeDeleteResps.getFailure());
            // 资源池 资源池分区 规格对应资源池ID
        ...
        if (standardSynchronizeDeleteResps.getSuccess().size() > 0) {
            Map<String, Object> updateMaps = new HashMap<String, Object>();
            updateMaps
                    .put("StandardSynchronizeResult", standardSynchronizeDeleteResps.getSuccess());
            // 资源池 资源池分区 规格对应资源池ID
        ...
        batchVOs.add(new BatchVO("MOD", "deleteStandardIso", standardId));

        try {
            ibatisDAO.updateBatchData(batchVOs);
            logger.info("删除虚拟机规格成功！");
        } catch (SQLException e) {
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION, "删除虚拟机规格失败", e);
            resultPath = ConstantEnum.ERROR.toString();
        } catch (Exception e) {
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION, "删除虚拟机规格失败", e);
            resultPath = ConstantEnum.ERROR.toString();
    ...
    /**
     * updateData 更新数据(未同步过,删除虚拟机规格信息)
     * 【未调用接口更新操作】
     * @param standardId 规格ID
     */
    private void updateDataStandard(String standardId) {
        
        int standardIsoNumber = 0;
        try {
            standardIsoNumber = (Integer)ibatisDAO.getSingleRecord("queryCountStandardIso", standardId);
        } catch (SQLException e) {
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION, "删除虚拟机规格失败", e);
            resultPath = ConstantEnum.ERROR.toString();
        } catch (Exception e) {
        ...
        try {
            ...
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION,
                    "删除虚拟机规格失败！原因：查询规格是否存在未同步过时，数据库异常。", e);
            resultPath = ConstantEnum.ERROR.toString();
        } catch (Exception e) {
            logger.error(CommonStatusCode.DATABASE_OPERATION_ECXEPTION,
                    "删除虚拟机规格失败！原因：查询规格是否存在未同步过时，数据库异常。", e);
