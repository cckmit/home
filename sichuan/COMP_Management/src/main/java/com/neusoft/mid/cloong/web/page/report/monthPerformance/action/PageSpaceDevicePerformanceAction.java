 ...
 */
public class PageSpaceDevicePerformanceAction extends MonthBaseAction {
	...
	public String execute() {
		logger.info("月度性能统计-page spcae使用率统计，开始");
		...
		try {
			...
			if ("0".equals(monthPerformanceInfo.getDeviceType())) { // 统计物理机内存
				chartDatalist = ibatisDAO.getData("getPmPagePerformance",
						monthPerformanceInfo);
			} else if ("1".equals(monthPerformanceInfo.getDeviceType())) { // 统计虚拟机内存
				chartDatalist = ibatisDAO.getData("getVmPagePerformance",
						monthPerformanceInfo);
			}

		} catch (SQLException e) {
			logger.error("月度性能统计-page spcae使用率统计，操作数据库异常", e);
			e.printStackTrace();
		}
		...
		logger.info("月度性能统计-page spcae使用率统计，结束");
