 ...
 */
public class DiskHistoryPerformanceAction extends HistoryBaseAction {

	private static final long serialVersionUID = 1L;

	private static Logger logger = Logger.getLogger(DiskHistoryPerformanceAction.class);
	...
	public String execute() {
		...
		try {
			if (null != historyPerformanceInfo.getStaFlag() && !"".equals(historyPerformanceInfo.getStaFlag())) {
				chartDatalist = ibatisDAO.getData("getDiskHistoryPerformance", historyPerformanceInfo);
			}
			
			appList = ibatisDAO.getData("getAppList", null);
		} catch (SQLException e) {
			logger.error("业务性能统计-磁盘使用率统计，操作数据库异常", e);
			e.printStackTrace();
		}
	...
	private List<String> JsonPackaging(List<HistoryPerformanceInfo> list) {
		...
		for (HistoryPerformanceInfo i : list) {
			...
			/* 平均使用率 */
			obj = new JSONObject();
			obj.put("y", i.getDiskAve());
			obj.put("deviceId", "");
			aveData.add(obj);

			/* 最大使用率 */
			obj = new JSONObject();
			obj.put("y", i.getDiskMax());
			obj.put("deviceId", i.getDiskMaxId());
			maxData.add(obj);

			/* 最小使用率 */
			obj = new JSONObject();
			obj.put("y", i.getDiskMin());
			obj.put("deviceId", i.getDiskMinId());
			minData.add(obj);
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public String getDiskHistoryPerformanceByAjax() {
		/* 查询日期处理 */
		this.formatDateForSql();
		
		if (null != historyPerformanceInfo.getStaFlag() && !"".equals(historyPerformanceInfo.getStaFlag())) {
			historyPerformanceInfolist = getPage("getHistoryPerformanceCount",
					"getDiskHistoryPerformance", historyPerformanceInfo,
