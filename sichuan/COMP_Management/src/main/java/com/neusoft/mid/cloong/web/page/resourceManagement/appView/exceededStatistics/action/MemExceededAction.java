 ...
 */
public class MemExceededAction extends ResourceManagementBaseAction {

	private static final long serialVersionUID = -2135178583729376110L;

	private static Logger logger = Logger.getLogger(MemExceededAction.class);
	...
	public String execute() {
		logger.info("所有业务-内存使用率统计，开始");

		List<DeviceExceededNum> perlist = null; // 按使用率超标设备数占比统计List
		List<DeviceExceededNum> numlist = null; // 按使用率超标设备数统计List

		try {
			perlist = ibatisDAO.getData("getMemOverNumByApp", deviceOverInfo);
			if (perlist.size() >= 10) {
				perlist = perlist.subList(0, 10);
			}

			numlist = ibatisDAO.getData("getMemOverNumByAppOrderByNum", deviceOverInfo);
			if (numlist.size() >= 10) {
				numlist = numlist.subList(0, 10);
			}
		} catch (SQLException e) {
			logger.error("所有业务-内存使用率统计，操作数据库异常", e);
			e.printStackTrace();
		}
	...
	private List<String> JsonPackaging(List<DeviceExceededNum> list) {
		...
		for (DeviceExceededNum i : list) {
			...
			/* 未超标 */
			obj = new JSONObject();
			obj.put("y", i.getMemNotOverNum());
			obj.put("per", i.getMemNotOverPer());
			obj.put("ave", i.getMemNotOverAve());
			obj.put("freeAve", i.getMemNotOverFreeAve());
			notOverData.add(obj);
			
			/* 超标 */
			obj = new JSONObject();
			obj.put("y", i.getMemOverNum());
			obj.put("per", i.getMemOverPer());
			obj.put("ave", i.getMemOverAve());
			obj.put("freeAve", i.getMemOverFreeAve());
			overData.add(obj);
		}
	...
	public String getPerListByAjax() {
		exceededlist = getPage("getMemOverNumCountByApp", "getMemOverNumByApp",
	...
	public String getNumListByAjax() {
		exceededlist = getPage("getMemOverNumCountByApp",
				"getMemOverNumByAppOrderByNum", deviceOverInfo,
