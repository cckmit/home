 ...
 */
public class CpuHistoryPerformanceAction extends HistoryBaseAction {
	...
	public String execute() {
		logger.info("业务性能统计-CPU使用率统计，开始");
		...
		try {
			if (null != historyPerformanceInfo.getStaFlag() && !"".equals(historyPerformanceInfo.getStaFlag())) {
				chartDatalist = ibatisDAO.getData("getCpuHistoryPerformance", historyPerformanceInfo);
			}
			
			appList = ibatisDAO.getData("getAppList", null);
		} catch (SQLException e) {
			logger.error("业务性能统计-CPU使用率统计，操作数据库异常", e);
			e.printStackTrace();
		}
		...
		logger.info("业务性能统计-CPU使用率统计，结束");
	...
	private List<String> JsonPackaging(List<HistoryPerformanceInfo> list) {
		...
		for (HistoryPerformanceInfo i : list) {
			...
			/* 平均使用率 */
			obj = new JSONObject();
			obj.put("y", i.getCpuAve());
			obj.put("deviceId", "");
			aveData.add(obj);

			/* 最大使用率 */
			obj = new JSONObject();
			obj.put("y", i.getCpuMax());
			obj.put("deviceId", i.getCpuMaxId());
			maxData.add(obj);

			/* 最小使用率 */
			obj = new JSONObject();
			obj.put("y", i.getCpuMin());
			obj.put("deviceId", i.getCpuMinId());
			minData.add(obj);
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public String getCpuHistoryPerformanceByAjax() {
		/* 查询日期处理 */
		this.formatDateForSql();
		
		if (null != historyPerformanceInfo.getStaFlag() && !"".equals(historyPerformanceInfo.getStaFlag())) {
			historyPerformanceInfolist = getPage("getHistoryPerformanceCount",
					"getCpuHistoryPerformance", historyPerformanceInfo,
