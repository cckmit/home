...
public class LoginAction extends BaseAction {
	...
	public String execute() {
		...
		try {
			...
			logger.info("用户登录的userId：" + tempUserId);
		...
		String sessionRS = (String) ActionContext.getContext().getSession()
				.get("random");
		Boolean verifyFlag = verify.equals(sessionRS);
		...
		if (null != userInfo) { // 用户ID存在
			try {
				 ...
				 * String passwordBase64Bak = new
				 * String(dec.decodeBuffer(base64Password));//解密
				 */
				logger.info("md5Password=" + md5Password);
				logger.info(Base64.decode(userInfo.getPassword()));
				if (!verifyFlag
						|| !md5Password.equalsIgnoreCase(Base64.decode(userInfo
	...
	private boolean validateInputPara() {
		...
		if (null == aesUserId || null == md5Password || ("").equals(aesUserId)
				|| ("").equals(md5Password)) {
