 ...
 */
public class DiskUsedPerListAction extends ResourceManagementBaseAction
{

	private static final long serialVersionUID = -5024421508278442852L;

	private static Logger logger = Logger.getLogger(DiskUsedPerListAction.class);
	...
	public String execute()
	{
		...
		//查询磁盘使用率列表，async异步，sync同步
		if(device_type.equals("2")){
			performancelist = getPage("queryPmDiskUsedPerListCount" , "queryPmDiskUsedPerList",
					staDeviceWeek,PageTransModel.ASYNC,PageToDisplayPerModel.PAGESIZE_10_DISPLAY);
		}else if(device_type.equals("3")){
			performancelist = getPage("queryVmDiskUsedPerListCount" , "queryVmDiskUsedPerList",
					staDeviceWeek,PageTransModel.ASYNC,PageToDisplayPerModel.PAGESIZE_10_DISPLAY);
		}else if(device_type.equals("0")){
			performancelist = getPage("queryMiniPmDiskUsedPerListCount" , "queryMiniPmDiskUsedPerList",
					staDeviceWeek,PageTransModel.ASYNC,PageToDisplayPerModel.PAGESIZE_10_DISPLAY);
		}else if(device_type.equals("1")){
			performancelist = getPage("queryMiniPmParDiskUsedPerListCount" , "queryMiniPmParDiskUsedPerList",
					staDeviceWeek,PageTransModel.ASYNC,PageToDisplayPerModel.PAGESIZE_10_DISPLAY);
		}
