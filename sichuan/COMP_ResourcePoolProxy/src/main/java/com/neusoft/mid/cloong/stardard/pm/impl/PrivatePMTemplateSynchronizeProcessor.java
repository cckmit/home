 ...
 * @author he.jf
 */
public class PrivatePMTemplateSynchronizeProcessor extends StandardBaseProcessor {

    /**
     * 日志
     */
    private static LogService logger = LogService
            .getLogger(PrivatePMTemplateSynchronizeProcessor.class);
     ...
     * 资源类型
     */
    private String resourceType = "CPC-RT-SRV";
    ...
    public String process(RuntimeContext req, RuntimeContext resp) {
        ...
        try {
            ...
            // 要保存到数据库的信息集合
            List<StandardSynchronizedState> willSynInfoList = queryWillSyncInfos(resp,
                    standardQueryPara);
            ...
            // 查询出所有要同步的信息
            List<StandardSynchronizedState> standardStates = querySycnTemplateList(resp,
                    standardQueryPara);

            // 向资源池发送消息
            List<TemplateSynchronizeResult> templateSynState = createTmeplateToResourcePool(req,
                    existTemplateMap, standardStates);
            ...
            for (StandardSynchronizedState state : standardStates) {
                ...
                if (resourceInfo == null || resourceInfo.getResourcePoolUrl() == null
                    ...
                    logger.error(CommonStatusCode.INTERNAL_ERROR,
                            "向资源池发送物理机操作请求失败，资源池代理系统内部错误，资源池配置错误", null);
                    ...
                    templateSysResult.setTemplateId(existTemplateMap.get(state.getStandardId()
                            + state.getOsId()));
                ...
                if (CREATE_REGEX.matcher(state.getSynchronizedState()).matches()) {
                    pmTemplateCreate(existTemplateMap, state, req, resourceInfo, templateSynState);
                }
    ...
    protected List<StandardSynchronizedState> querySycnTemplateList(RuntimeContext resp,
        ...
        try {
            ...
            standardStates = ibatisDao.getData("queryPmTemplateSynchronizedState",
                    standardQueryPara);
    ...
    protected Map<String, String> queryExsitTmplates(RuntimeContext req, RuntimeContext resp)
        ...
        try {
            ...
            String standardId = (String) req.getAttribute("STANDARD_ID");
            List<Template> existTemplateList = ibatisDao
                    .getData("queryTemplateMessage", standardId);
            // 将模板信息list转换成map
     ...
     * @throws StandardOperationException 规格操作异常
     */
    private List<StandardSynchronizedState> queryWillSyncInfos(RuntimeContext resp,
            StandardQueryPara standardQueryPara) throws StandardOperationException {
        // 查询出要保存到数据库关联表和同步表的信息
        List<StandardSynchronizedState> willSynInfoList;
        try {
            logger.info("开始查询出要保存到数据库关联表和同步表的信息!");
            willSynInfoList = ibatisDao.getData("querySaveDbPMSynchronized", standardQueryPara);
        } catch (SQLException e) {
    ...
    private void saveTmplateAndSyncInfoToDB(List<StandardSynchronizedState> willSynInfoList,
        ...
        for (StandardSynchronizedState willSyncTemplateInfo : willSynInfoList) {
            String willSyncTemplateKey = willSyncTemplateInfo.getStandardId()
                    + willSyncTemplateInfo.getOsId();
            if ("".equals(existTemplateMap.get(willSyncTemplateKey))
                ...
                String templateId = transactonGen.generatorPMItemId();
                ...
                existTemplateMap.put(newTemplate.getStandardId() + newTemplate.getOsId(),
                        newTemplate.getTemplateId());
                // 掉用封装模板同步实体
                StandardSynInfo standardSynInfo = this.saveStandardSynInfo(req, existTemplateMap,
                        willSyncTemplateInfo);
     ...
     */
    private void pmTemplateCreate(Map<String, String> templateMap, StandardSynchronizedState state,
            RuntimeContext req, ResourcePoolInfo resourceInfo,
            List<TemplateSynchronizeResult> templateSynState) {
        ...
        String cpuType = (String) req.getAttribute("CPU_TYPE");
        String memorySize = (String) req.getAttribute("MEMORY_SIZE");
        String storageSize = (String) req.getAttribute("STORAGE_SIZE");
        String EthAdaNum = (String) req.getAttribute("ETH_ADA_NUM");
        String EthAdaType = (String) req.getAttribute("ETH_ADA_TYPE");
        String SCSIAdaNum = (String) req.getAttribute("SCSI_ADA_NUM");
        String HBANum = (String) req.getAttribute("HBA_NUM");
        String HBAType = (String) req.getAttribute("HBA_TYPE");
        ...
        pmReq.setCpuType(cpuType);
        pmReq.setMemorySize(memorySize);
        pmReq.setStorageSize(storageSize);
        pmReq.setEthAdaNum(EthAdaNum);
        pmReq.setEthAdaType(EthAdaType);
        pmReq.setSCSIAdaNum(SCSIAdaNum);
        pmReq.setHBANum(HBANum);
        pmReq.setHBAType(HBAType);
        ...
        // 更新状态
        TemplateSynchronizeResult templateSysResult = new TemplateSynchronizeResult();
        templateSysResult.setResourcePoolId(pmReq.getResourcePoolId());
        templateSysResult.setResourcePoolPartId(pmReq.getResourcePoolPartId());
        templateSysResult.setStandardId(state.getStandardId());
        ...
        templateSysResult.setState(pmResp.getResultCode().equals(RESP_SUCCESS_CODE) ? "0" : "1");
    ...
    private StandardQueryPara assmbleTemplatePara(RuntimeContext req) {
        ...
        List<Map<String, String>> syncPoolInfoMaps = (List<Map<String, String>>) req
                .getAttribute("RESOURCE_POOLS");

        StandardQueryPara para = new StandardQueryPara();
        para.setStandardId(standardId);
        List<ResourcePoolInfoPara> syncPoolInfoList = new ArrayList<ResourcePoolInfoPara>();
        for (Map<String, String> syncResourcePoolPartMap : syncPoolInfoMaps) {
            ResourcePoolInfoPara syncPoolInfo = new ResourcePoolInfoPara();
            syncPoolInfo.setResourcePoolId(syncResourcePoolPartMap.get("RES_POOL_ID"));
            syncPoolInfo.setResourcePoolPartId(syncResourcePoolPartMap.get("RES_POOL_PART_ID"));
            syncPoolInfoList.add(syncPoolInfo);
        }
        para.setPoolInfos(syncPoolInfoList);
        
        @SuppressWarnings("unchecked")
        List<Map<String, String>> syncOSlInfoMaps = (List<Map<String, String>>) req
                .getAttribute("OSINFOS");
        List<ResourcePoolInfoPara> syncOsInfoList = new ArrayList<ResourcePoolInfoPara>();
        for (Map<String, String> syncOsPartMap : syncOSlInfoMaps) {
            ResourcePoolInfoPara syncOsInfo = new ResourcePoolInfoPara();
            syncOsInfo.setOsId(syncOsPartMap.get("OS_ID"));
            syncOsInfoList.add(syncOsInfo);
        }
        para.setOsInfos(syncOsInfoList);
        return para;
    }
     ...
     * @throws SQLException
     */
    private void insertInfos(List<Template> templateInfos, List<StandardSynInfo> synchronizedInfos)
            throws SQLException {
     ...
     * @throws StandardOperationException 规格操作异常
     */
    private List<Map<String, String>> updateSynState(
            List<TemplateSynchronizeResult> templateSynState, RuntimeContext resp)
            throws StandardOperationException {
     ...
     * @return 模板同步信息
     */
    private StandardSynInfo saveStandardSynInfo(RuntimeContext req,
            Map<String, String> templateMap, StandardSynchronizedState save) {
        ...
        standardSynInfo.setStandardType("1");
