/*******************************************************************************
 * @(#)PMStandardModifyImpl.java 2013-3-18
 *
...
import com.neusoft.mid.cloong.stardard.pm.PMStandardModify;
 ...
 * @version $Revision 1.1 $ 2013-3-18 下午03:29:01
 */
public class PMStandardModifyImpl implements PMStandardModify {
    /**
     * 向资源池进行请求的公用类，用户创建客户端类和组装响应信息
     */
    private RequestCommon common;

    private static LogService logger = LogService.getLogger(PMStandardModifyImpl.class);
    ...
    @Override
    public PMStandardModifyResp modifyPMStandard(PMStandardModifyReq req) {
        // 打开发送客户端
        ...
        PMStandardModifyResp resp = new PMStandardModifyResp();
        ...
        sb.append("向资源池发送规格编号为[" + req.getStandardId() + "]的物理机操作请求成功，获取的响应码为[").append(
    ...
    private PMStandardModifyResp assmbleErrorResp(InterfaceResultCode code) {
        PMStandardModifyResp resp = new PMStandardModifyResp();
    ...
    private String generateStandardXml(PMStandardModifyReq req) {
