 ...
 */
public class PrivateEBSStandardSynchronizeProcessor extends StandardBaseProcessor {

    private static LogService logger = LogService
            .getLogger(PrivateEBSStandardSynchronizeProcessor.class);

    @Value("${template.bs.defaultVal.TIER}")
    private String defaultTier;

    @Value("${template.bs.defaultVal.RAID}")
    private String defaultRaid;

    @Value("${template.bs.defaultVal.STORAGE_NET}")
    private String defaultStorageNet;

    @Value("${template.bs.defaultVal.VOL_NUM}")
    private String defaultVolNum;
     ...
     * 资源类型
     */
    private String resourceType = "CPC-RT-BS";
        ...
        for (StandardSynchronizedState state : standardStates) {
            ...
            if (resourceInfo == null || resourceInfo.getResourcePoolUrl() == null) {
                logger.error(CommonStatusCode.INTERNAL_ERROR,
                        "向资源池发送虚拟硬盘操作请求失败，资源池代理系统内部错误，资源池配置错误", null);
            ...
            if (CREATE_REGEX.matcher(state.getSynchronizedState()).matches()) {
                ...
                String ebsResourceType = (String) req.getAttribute("RESOURCE_TYPE");
                String tier = req.getAttribute("TIER") == null ? this.defaultTier : (String) req
                        .getAttribute("TIER");
                String raid = req.getAttribute("RAID") == null ? this.defaultRaid : (String) req
                        .getAttribute("RAID");
                String storageNet = req.getAttribute("STORAGE_NET") == null ? this.defaultStorageNet
                        : (String) req.getAttribute("STORAGE_NET");
                String volNum = req.getAttribute("VOL_NUM") == null ? this.defaultVolNum
                        : (String) req.getAttribute("VOL_NUM");
                ...
                ebsReq.setEbsResourceType(ebsResourceType);
                ebsReq.setTier(tier);
                ebsReq.setRaid(raid);
                ebsReq.setStorageNet(storageNet);
                ebsReq.setVolNum(volNum);
                ...
                ebsReq.setTransactionID(this.getTransactonGen().generateTransactionId(
                        ebsReq.getResourcePoolId(), ebsReq.getTimestamp()));

                // 向资源池发送请求
                EBSStandardCreateResp ebsResp = create.createEBSStandard(ebsReq);
                resultMap.put("STATE", ebsResp.getResultCode().equals(RESP_SUCCESS_CODE) ? "0"
                        : "1");
