...
import com.neusoft.mid.cloong.rpws.private1072.snapshot.ApplySnapshotServiceReq;
import com.neusoft.mid.cloong.rpws.private1072.snapshot.ApplySnapshotServiceResp;
...
public class SnapshotApply {
     ...
     * @return VMModifyResp
     */
    public RPPApplySnapshotResp apply(SnapshotApplyReq req) {
        ...
        try {
            ...
            // 生成请求报文体
        	ApplySnapshotServiceReq webserviceReqBody = genWebserviceReqBody(req);
            ...
            // 发送
            ApplySnapshotServiceResp webserviceRespBody = client.applySnapshotService(webserviceReqBody, webserviceReqHeader,
     ...
     * @return
     */
    private ApplySnapshotServiceReq genWebserviceReqBody(SnapshotApplyReq req) {

    	ApplySnapshotServiceReq webserviceReqBody = new ApplySnapshotServiceReq();
    	/*private String vmId;
        ...
        webserviceReqBody.setSnapshotName(req.getInfo().getSnapshot_name());
        webserviceReqBody.setSnapshotDesc(req.getInfo().getSnapshot_desc());
        webserviceReqBody.setGenerateMemory(req.getInfo().getGenerate_memory());
        webserviceReqBody.setQuiesce(req.getInfo().getQuiesce());
     ...
     * @param modifyVMResp
     */
    private void assembleResp(RPPApplySnapshotResp resp, ApplySnapshotServiceResp webserviceRespBody, String resultCode) {
        resp.setResultMessage(webserviceRespBody.getFaultString());
        resp.setResultCode(resultCode);
        resp.setFaultstring(webserviceRespBody.getFaultString());

        resp.setSnapshotIdList(new ArrayList<String>());
        if (webserviceRespBody.getSnapshotId() != null && webserviceRespBody.getSnapshotId()!= null) {
            resp.getSnapshotIdList().addAll(webserviceRespBody.getSnapshotId());
        }
    }
     ...
     * @return resp
     */
    private RPPApplySnapshotResp assmbleErrorResp(InterfaceResultCode code) {
    	RPPApplySnapshotResp resp = new RPPApplySnapshotResp();
